...
class OrganizationManagementView(QDialog):
    ...
    def __init__(self,parent=None):
        super().__init__(parent); self.setWindowTitle("Gerenciar Organizações"); self.resize(1020,560); root=QVBoxLayout(self); self.search=QLineEdit(); self.search.setPlaceholderText("Buscar organização..."); self.search.textChanged.connect(self._filter); root.addWidget(self.search); self.table=QTableWidget(0,11); self.table.setHorizontalHeaderLabels(["Nome","Perfil","Papel","Documentos","Pastas","Membros","Nuvem","Status","Criada em","Última atividade","Descrição"]); self.table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.ResizeToContents); self.table.horizontalHeader().setStretchLastSection(True); root.addWidget(self.table)
    ...
    def set_items(self,items):
        ...
        for organization,membership,stats in items:
            row=self.table.rowCount(); self.table.insertRow(row); profile=OrganizationFeatureService().for_organization(organization).profile_name; values=(organization.name,profile,membership.role,stats["documents"],stats["folders"],stats["members"],stats["cloud"],"ARQUIVADA" if organization.archived_at else organization.status,organization.created_at,stats["last_activity"] or "—",organization.description or "")
