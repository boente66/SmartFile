...
class OrganizationAdminService:
    ...
    def duplicate(self,organization_id,new_name,activate=False):
        ...
        with self.database.transaction():
            target=self.organizations.create(new_name,source.description,source.template_code,source.storage_plan_code); target.icon=source.icon; target.color=source.color; target.profile_code=source.profile_code; self.organizations.repository.update(target)
